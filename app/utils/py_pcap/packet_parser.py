...
def __parse_ip_packet(data: bytes) -> tuple:
    ...
    return bytes_to_ip(src_ip), src_port, bytes_to_ip(dst_ip), dst_port, protocol_numbers.get(prot, '')
...
def __parse_ipv6_packet(data: bytes) -> tuple:
    ...
    return bytes_to_ip(src_ip), src_port, bytes_to_ip(dst_ip), dst_port
...
def parse_packet(packet_data: bytes) -> np.array:
    try:
        ...
        return globals()[f'__parse_{[var.rsplit('_')[-1].lower() for var in dir(socket) if 'ETHERTYPE' in var and eth_type == getattr(socket, var)][0]}_packet'](packet_data[14:])
    except struct.error:
